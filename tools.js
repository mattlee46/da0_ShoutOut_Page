...
const get_slack_info = (code) =>{
  console.log("codetools::"+code);

  fetch('https://slack.com/api/openid.connect.token?client_id=2546915667.4490171627924&client_secret=c6fd9cc008d5e927378087ef6c49514c&code='+code+'&grant_type=authorization_code').then(function(response_epic) {
                  // 直接轉成JSON格式
    //picchu
    //https://slack.com/api/openid.connect.token?client_id=1588523441350.5015726780389&client_secret=dbf21a6b1a9b755cbde32a79e95dc660&code='+code+'&grant_type=authorization_code
    //g0v
    //'https://slack.com/api/openid.connect.token?client_id=2546915667.4490171627924&client_secret=c6fd9cc008d5e927378087ef6c49514c&code='+code+'&grant_type=authorization_code'
      return response_epic.json()
  }).then(function(obj2) {
    console.log("ok:::::"+(obj2['ok']));
    console.log("error:::::"+(obj2['error']));
    console.log("access_token:::::"+(obj2['access_token']));
    console.log("id_token:::::"+(obj2['id_token']));

    var token = obj2['id_token'];

    console.log("token:::::"+(obj2['id_token']));
    
    var decoded = jwt_decode(token);
    console.log("decoded:::"+JSON.stringify(decoded));
     
    console.log("decoded:::"+decoded['name']);
    console.log("decodedIDIDID:::"+decoded['sub']);
    // $("#C1").text(decoded['name']);
    $("#slack_login_btn").hide();
    $("#back_to_home").show();
    get_shoutout_from_userid(decoded['sub']);
    $("#connect_text").hide();
    $("#after_connect").show();
    $("#shoutouts_g0v").hide();
    $("#shoutouts").show();
    $("#epic_title").text(decoded['name']+'! These are your shoutouts from community');
...
const back_to_home = () =>{
    ...
    $("#slack_login_btn").show();
    $("#C1").text("Shoutouts you received");
    $("#back_to_home").hide();
    $("#connect_text").show();
    $("#after_connect").hide();
    $("#shoutouts").hide();
...
const get_shoutout_from_userid = (id) =>{
  fetch('https://epic-slack-app.herokuapp.com/api/json/users/by-id/'+id+'/epic002').then(function(response_epic) {
      // 直接轉成JSON格式
    //https://epic-slack-app.herokuapp.com/api/json/users/by-id/U049T974GDQ/epic002
    //https://epic-slack-app.herokuapp.com/api/json/users/by-id/'+id+'/epic002  
      return response_epic.json()
  }).then(function(obj2) {
    
      obj2['epic002'].sort((a, b) => a['ts'] - b['ts']);

      // console.log('111:'+JSON.stringify(obj2['epic002']));
      ...
      for (var t = obj2['epic002'].length-1; t >= 0  ; t--) {
        ...
        var message1 = String(obj2['epic002'][t]['text']).replace(/</g, ' ');
        var message = message1.replace(/>/g, '  ');

      

        let unix_timestamp = Number(obj2['epic002'][t]['ts']);
        ...
        var formattedTime = date.toDateString().substr(-11);
      ...
      for(var k=0 ; k<obj2['epic002'].length ; k++ ){
        // epic.push('\n第'+(k+1)+'則史詩\n');
        // epic.push('no.'+(k+1)+' shoutouts\n'+String(epic_notSort[k])+'\n\n');
        // var A = '<div class="alert justify-content-start col-lg-8 col-11" role="alert" style="background-color:#595959;border-radius: 25px;text-align: left;font-family: "Satoshi", sans-serif;font-weight: bold; word-wrap: break-word; white-space: pre-wrap;"><p style="font-size: 20px;font-weight: bold;">'+String(epic_ts[k])+'<br></p><p style="font-size: 20px;font-weight: bold;">'+String(epic_notSort[k])+'</p><br><p style="font-size: 14px;font-weight: bold;text-align: right;">from '+String(epic_user[k])+' at #'+String(epic_channel[k])+'</p></div>';
        var A = '<div class="alert justify-content-start col-lg-8 col-11" role="alert" style="background-color:#595959;border-radius: 25px;text-align: left;font-family: "Satoshi", sans-serif;font-weight: bold; word-wrap: break-word; white-space: pre-wrap;"><p style="font-size: 20px;font-weight: bold;">'+shouout_obj['time'][k]+'<br></p><p style="font-size: 20px;font-weight: bold;word-wrap: break-word;">'+shouout_obj['message'][k]+'</p><br><p style="font-size: 14px;font-weight: bold;text-align: right;">from '+shouout_obj['user'][k]+' at #'+shouout_obj['channel'][k]+'</p></div>';
        //change slack emoji
        var Big = emojione.toImage(A);   
        //resize emoji     
        var C = Big.replace(/class="emojione"/g, 'class="emojione" style="width: 25px; height: 25px;"');
      ...
      if (String(epic[0]) === 'undefined') {
        ...
        console.log('text:::'+String(obj2['epic002']));
         ...
         console.log('length:::'+obj2['epic002'].length);
         $("#score").text((obj2['epic002'].length));
         //<img style="width: 16px;" src="https://emoji.slack-edge.com/T02G2SXKM/jump-g0v/af32f5764ec85d2f.gif">
      }
...
const get_shoutout_g0v = () =>{
  fetch('https://epic-slack-app.herokuapp.com/api/json/users/by-id/U04FCLTTECE/epic002').then(function(response_epic) {
      // 直接轉成JSON格式
    //https://epic-slack-app.herokuapp.com/api/json/users/by-id/U049T974GDQ/epic002
    //https://epic-slack-app.herokuapp.com/api/json/users/by-id/'+id+'/epic002  
      return response_epic.json()
      ...
      for(var k=0 ; k<obj2['epic002'].length ; k++ ){
        ...
        var A = '<div class="alert justify-content-start col-lg-8 col-11" role="alert" style="background-color:#595959;border-radius: 25px;text-align: left;font-family: "Satoshi", sans-serif;font-weight: bold; word-wrap: break-word; white-space: pre-wrap;"><p style="font-size: 20px;font-weight: bold;">'+shouout_obj['time'][k]+'<br></p><p style="font-size: 20px;font-weight: bold;word-wrap: break-word;">'+shouout_obj['message'][k]+'</p><br><p style="font-size: 14px;font-weight: bold;text-align: right;">from '+shouout_obj['user'][k]+' at #'+shouout_obj['channel'][k]+'</p></div>';
         // const B = emojione.shortnameToImage(A);
        // console.log("AAA:::"+A);
        var Big = emojione.toImage(A);
         // console.log("Big:::"+Big);
        var C = Big.replace(/class="emojione"/g, 'class="emojione" style="width: 25px; height: 25px;"');
       ...
       // const epic[k] = emojione.shortnameToImage(epic[k]);

        $("#epic_g0v").append(epic[k]);
      ...
      if (String(epic[0]) === 'undefined') {
        ...
        console.log('text:::'+String(obj2['epic002']));
         ...
         $("#shoutouts_total").text((obj2['epic002'].length));
